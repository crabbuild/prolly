...
impl PutItem {
    ...
    pub fn item(mut self, name: impl Into<String>, value: AttributeValue) -> Self {
        self.item.insert(name.into(), value);
        self
    }

    pub fn set_item(mut self, item: Option<HashMap<String, AttributeValue>>) -> Self {
        self.item = item.unwrap_or_default();
        self
    }
    ...
    pub async fn send(self) -> Result<PutItemOutput> {
        Ok(self.send_with_metadata().await?.output)
    }

    #[tracing::instrument(
        name = "prolly_dynamodb.PutItem",
        level = "debug",
        skip_all,
        fields(db_system = "dynamodb", db_operation = "PutItem"),
        err
    )]
    pub async fn send_with_metadata(self) -> Result<WithMetadata<PutItemOutput>> {
        let table = self
            .table_name
            .ok_or_else(|| Error::InvalidRequest("PutItem.table_name is required".into()))?;
        if self.item.is_empty() {
            return Err(Error::InvalidRequest("PutItem.item is required".into()));
        }
            ...
            match condition {
                Some(condition) => {
                    if return_old {
                        self.client
                            .core()
                            .put_item_conditionally_with_old(
                                &table,
                                item,
                                expected_head.as_ref(),
                                &condition,
                ...
                None => {
                    if return_old {
                        self.client
                            .core()
                            .put_item_with_old(&table, item, expected_head.as_ref())
                            .await
                            .map_err(|error| {
                                conditional_error_from_core(error, return_failure_old)
                            })?
                        ...
                        self.client
                            .core()
                            .put_item_result(&table, item, expected_head.as_ref())
                            .await
                            .map_err(|error| {
                                conditional_error_from_core(error, return_failure_old)
                            })?
        ...
        Ok(WithMetadata::single_write(
            PutItemOutput::builder().set_attributes(attributes).build(),
            table,
            version_id,
            result.commit_id,
            transition,
