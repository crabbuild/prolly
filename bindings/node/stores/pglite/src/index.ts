...
export interface PGliteStoreOptions { readonly adapterName?: string; readonly readParallelism?: number; }

export class PGliteStore implements RemoteStore {
  readonly #database: PGliteInterface;
  readonly #descriptor: StoreDescriptor;
  readonly #pending = new Set<Promise<unknown>>();
  #accepting = true;

  constructor(database: PGliteInterface, options: PGliteStoreOptions = {}) {
    if (database == null) throw new StoreError("invalid_argument", "PGlite database is required");
    this.#database = database;
    this.#descriptor = validateStoreDescriptor({ protocolMajor: 1, adapterName: options.adapterName?.trim() || "pglite-v1", provider: "pglite", schemaVersion: 1, capabilities: { nativeBatchReads: true, atomicBatchWrites: true, nodeScan: true, hints: true, atomicNodesAndHint: true, rootScan: true, rootCompareAndSwap: true, transactions: true, readParallelism: options.readParallelism ?? 16 }, limits: {} });
  }

  async initializeSchema(signal?: AbortSignal): Promise<void> { return this.#run("initialize_schema", signal, async () => { throwIfAborted(signal); await this.#database.exec(PGLITE_SCHEMA_SQL); throwIfAborted(signal); }); }
  ...
  async batchNodes(operations: readonly NodeMutation[], signal?: AbortSignal): Promise<void> { const owned = operations.map(cloneNodeMutation); return this.#run("batch_nodes", signal, () => this.#database.transaction(async (tx) => applyNodeMutations(tx, owned, signal))); }
  async batchGetNodesOrdered(cids: readonly Uint8Array[], signal?: AbortSignal): Promise<OptionalBytes[]> { const keys = cids.map(ownBytes); return this.#run("batch_get_nodes_ordered", signal, async () => { if (keys.length === 0) return []; const placeholders = keys.map((_, index) => `$${index + 1}`).join(", "); const result = await query<{ cid: Uint8Array; node: Uint8Array }>(this.#database, `SELECT cid, node FROM prolly_nodes WHERE cid IN (${placeholders})`, keys, signal); const values = new Map(result.rows.map(({ cid, node }) => [hex(cid), ownBytes(node)])); return keys.map((key) => values.has(hex(key)) ? presentBytes(values.get(hex(key))!) : missingBytes()); }); }
  async listNodeCids(signal?: AbortSignal): Promise<Uint8Array[]> { return this.#run("list_node_cids", signal, async () => (await query<{ cid: Uint8Array }>(this.#database, "SELECT cid FROM prolly_nodes ORDER BY cid", [], signal)).rows.map(({ cid }) => ownBytes(cid))); }
  async getHint(namespace: Uint8Array, key: Uint8Array, signal?: AbortSignal): Promise<OptionalBytes> { const ns = ownBytes(namespace); const ownedKey = ownBytes(key); return this.#run("get_hint", signal, () => queryOptional(this.#database, SELECT_HINT, [ns, ownedKey], signal)); }
  async putHint(namespace: Uint8Array, key: Uint8Array, value: Uint8Array, signal?: AbortSignal): Promise<void> { const ns = ownBytes(namespace); const ownedKey = ownBytes(key); const ownedValue = ownBytes(value); return this.#run("put_hint", signal, async () => { await query(this.#database, UPSERT_HINT, [ns, ownedKey, ownedValue], signal); }); }
  async batchPutNodesWithHint(nodes: readonly NodeEntry[], namespace: Uint8Array, key: Uint8Array, value: Uint8Array, signal?: AbortSignal): Promise<void> { const ownedNodes = nodes.map(({ cid, node }) => ({ cid: ownBytes(cid), node: ownBytes(node) })); const ns = ownBytes(namespace); const ownedKey = ownBytes(key); const ownedValue = ownBytes(value); return this.#run("batch_put_nodes_with_hint", signal, () => this.#database.transaction(async (tx) => { for (const node of ownedNodes) await query(tx, UPSERT_NODE, [node.cid, node.node], signal); await query(tx, UPSERT_HINT, [ns, ownedKey, ownedValue], signal); })); }
  ...
  async compareAndSwapRootManifest(name: Uint8Array, expected: OptionalBytes, replacement: OptionalBytes, signal?: AbortSignal): Promise<RootCasResult> { const key = ownBytes(name); const wanted = normalizeOptionalBytes(expected); const next = normalizeOptionalBytes(replacement); return this.#run("compare_and_swap_root_manifest", signal, () => this.#database.transaction(async (tx) => { await query(tx, LOCK_ROOT, [key], signal); const current = await queryOptional(tx, SELECT_ROOT_FOR_UPDATE, [key], signal); if (!optionalEqual(current, wanted)) return { applied: false, current }; await writeRoot(tx, key, next, signal); return { applied: true, current: normalizeOptionalBytes(next) }; })); }
  ...
  async commitTransaction(nodes: readonly NodeMutation[], conditions: readonly RootCondition[], roots: readonly RootWrite[], signal?: AbortSignal): Promise<StoreTransactionResult> { const ownedNodes = nodes.map(cloneNodeMutation); const ownedConditions = conditions.map(({ name, expected }) => ({ name: ownBytes(name), expected: normalizeOptionalBytes(expected) })); const ownedRoots = roots.map(cloneRootWrite); return this.#run("commit_transaction", signal, () => this.#database.transaction(async (tx) => { for (const name of uniqueSortedNames(ownedConditions.map(({ name }) => name))) await query(tx, LOCK_ROOT, [name], signal); for (const condition of ownedConditions) { const current = await queryOptional(tx, SELECT_ROOT_FOR_UPDATE, [condition.name], signal); if (!optionalEqual(current, condition.expected)) return { applied: false, conflict: { name: ownBytes(condition.name), expected: normalizeOptionalBytes(condition.expected), current } }; } await applyNodeMutations(tx, ownedNodes, signal); for (const root of ownedRoots) await writeRoot(tx, root.name, root.kind === "put" ? presentBytes(root.manifest) : missingBytes(), signal); return { applied: true }; })); }

  async #run<T>(operation: string, signal: AbortSignal | undefined, call: () => Promise<T>): Promise<T> { throwIfAborted(signal); if (!this.#accepting) throw new StoreError("internal", "PGlite store is closed"); const pending = call().then((value) => { throwIfAborted(signal); return value; }).catch((error: unknown) => { if (signal?.aborted) throw new StoreError("cancelled", "PGlite operation was cancelled", { cause: signal.reason }); throw mapPGliteError(operation, error); }); this.#pending.add(pending); try { return await pending; } finally { this.#pending.delete(pending); } }
}
...
function cloneNodeMutation(operation: NodeMutation): NodeMutation { return operation.kind === "upsert" ? upsertNode(operation.cid, operation.node) : deleteNode(operation.cid); }
function cloneRootWrite(write: RootWrite): RootWrite { return write.kind === "put" ? { kind: "put", name: ownBytes(write.name), manifest: ownBytes(write.manifest) } : { kind: "delete", name: ownBytes(write.name) }; }
function optionalEqual(left: OptionalBytes, right: OptionalBytes): boolean { return left.present === right.present && (!left.present || equalBytes(left.value, right.value)); }
...
function compareBytes(left: Uint8Array, right: Uint8Array): number { const common = Math.min(left.length, right.length); for (let index = 0; index < common; index++) { const compared = left[index]! - right[index]!; if (compared !== 0) return compared; } return left.length - right.length; }
...
function providerCode(error: unknown): string | undefined { if (typeof error !== "object" || error === null || !("code" in error) || typeof error.code !== "string") return undefined; return /^[0-9A-Z]{5}$/.test(error.code) ? error.code : undefined; }
function mapPGliteError(operation: string, error: unknown): StoreError { if (error instanceof StoreError) return error; const code = providerCode(error); const retryable = code === "40001" || code === "40P01" || code === "55P03"; return new StoreError(retryable ? "unavailable" : "internal", "PGlite operation failed", { retryable, providerCode: code === undefined ? undefined : `pglite:${code}:${operation}`, cause: error }); }
