...
- ordered boundaries, range cursors, reverse pages, and cursor-resumed diffs;
- named roots, manifest metadata, retained GC, large values, and blob GC;
- merge callbacks and JavaScript-owned custom stores;
- portable proof bundles, HMAC envelopes, and one-shot verification.

The native and async engines expose `parallelBatch`, `parallelBatchWithStats`,
`batchWithStats`, and `appendBatchWithStats` for parallel mutation application
plus route/write telemetry.

`defaultParallelConfig()` returns the Rust default parallel-batch
configuration for Node callers. Native helpers also expose config and encoding
constructors; the TypeScript facade mirrors those helper shapes for non-native
callers.
...
`snapshotBundleDigest*`, `snapshotBundleSummary*`, and
...
integrity checks outside the store.

Hint helpers include exact-key, prefix, and range changed-span constructors.
Batch helpers include upsert/delete mutation constructors. Merge helpers
include normal and CRDT resolution constructors plus built-in resolver helpers
for callback resolvers.
...
```sh
node bindings/node/examples/local-first-state.ts
```

Run all scenarios:

```sh
node bindings/node/examples/cookbook-scenarios.ts
```
