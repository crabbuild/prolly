...
| --- | --- |
| Core tree | `create`, `get`, `get_many`, `put`, `delete`, raw-byte half-open `delete_range(start, end)`, canonical-write stats, `batch`, batch stats, bulk build, sorted bulk build, append batch, parallel batch stats |
