...
| Source-bound canonical build and validation | `src/prolly/proximity/accelerator/turboquant.rs`, sync/async trusted-open root-count unit tests, and adaptive pre-encoding temporary-budget tests that preserve the canonical manifest while bounding each retained batch | Implemented |
