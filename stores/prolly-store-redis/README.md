...
native `AsyncProlly::indexed_map(...).await` through `RedisStore` and the
first-class `SyncRedisStore` facade with synchronous `Prolly::indexed_map`.
...
and Redis connection backpressure without blocking workers.
