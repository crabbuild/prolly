...
#[tokio::test(flavor = "multi_thread")]
async fn local_backend_supports_native_asynchronous_indexed_maps() {
    let temp = tempfile::tempdir().unwrap();
    let backend = TursoBackend::open(temp.path().join("async-indexed-map.db"))
        .await
        .unwrap();

    prolly::remote_conformance::assert_remote_backend_async_indexed_map_contract(backend).await;
}
