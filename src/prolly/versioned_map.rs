...
use super::secondary_index::indexed_collection_root_name;
...
#[allow(dead_code)]
pub(crate) enum MapWriteAuthority {
    Unmanaged,
}
...
{
    let canonical_root = indexed_collection_root_name(map_id)?;
    match (
        tx.load_named_root(&canonical_root).await?.is_some(),
        authority,
    ) {
        (true, MapWriteAuthority::Unmanaged) => Err(Error::IndexesRequireIndexedMap {
            map_id: map_id.to_vec(),
            active_indexes: Vec::new(),
        }),
        (false, MapWriteAuthority::Unmanaged) => Ok(()),
    }
}
...
{
    let canonical_root = indexed_collection_root_name(map_id)?;
    match (tx.load_named_root(&canonical_root)?.is_some(), authority) {
        (true, MapWriteAuthority::Unmanaged) => Err(Error::IndexesRequireIndexedMap {
            map_id: map_id.to_vec(),
            active_indexes: Vec::new(),
        }),
...
impl<'a, S: Store> MapSnapshot<'a, S> {
    pub(crate) fn new(prolly: &'a Prolly<S>, version: MapVersion) -> Self {
        Self { prolly, version }
    }
    ...
    pub(crate) fn from_tree(
        ...
        Ok(Self::new(prolly, MapVersion::new(tree, None, is_head)?))
    }
...
mod index_fence_tests {
    ...
    #[test]
    fn absent_collection_state_read_conflicts_with_later_activation() {
        ...
        let tree = engine
            .put(&engine.create(), b"format".to_vec(), vec![1])
            .unwrap();
        let root = indexed_collection_root_name(b"users").unwrap();
        engine.publish_named_root(&root, &tree).unwrap();
