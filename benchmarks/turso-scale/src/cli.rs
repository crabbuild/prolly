...
pub const USAGE: &str = "usage: prolly-turso-scale-bench [--profile smoke|full] [--output PATH] [--sizes LIST] [--runs N] [--operations LIST] [--patterns LIST] [--changes N|auto] [--read-samples N] [--min-free-gb N] [--tokio-workers N] [--keep-fixtures] [--revision REV] [--dirty|--clean]";
    ...
    S: AsRef<str>,
{
    ...
    let mut config = match profile.as_str() {
        "smoke" => RunConfig::smoke(PathBuf::from("performance-results/turso/baseline/smoke")),
        "full" => RunConfig::full(
            PathBuf::from("performance-results/turso/baseline"),
            "unknown".to_string(),
            true,
        ),
