...
pub fn write_report(
    ...
    let mut file = File::create(path)
        .map_err(|error| format!("failed to create {}: {error}", path.display()))?;
    writeln!(file, "# Turso prolly scale baseline\n").map_err(|error| error.to_string())?;
    writeln!(
        file,
    ...
    .map_err(|error| error.to_string())?;
    writeln!(file, "- Keys are 24 bytes and values are 100 bytes. Results do not predict Turso Cloud synchronization, concurrent writers, remote filesystems, or raw Turso SQL.")
        .map_err(|error| error.to_string())
}
