...
use crate::fixture::{namespace_key_count, redis_stats, FixtureLayout, RedisStats};
...
const BASE_ROOT: &[u8] = b"redis-scale/base";
const RESULT_ROOT: &[u8] = b"redis-scale/result";

type Manager = AsyncProlly<RedisStore>;

pub async fn build_fixture(
    spec: &FixtureSpec,
    layout: &FixtureLayout,
) -> Result<FixtureRow, String> {
    let source_prefix = layout.source_prefix();
    let store = open_store(layout.redis_url(), source_prefix.clone()).await?;
    ...
    let reopened_store = open_store(layout.redis_url(), source_prefix.clone()).await?;
    ...
    let redis = redis_stats(layout.redis_url()).await?;
    let namespace_keys = namespace_key_count(layout.redis_url(), &source_prefix).await?;
    ...
    Ok(FixtureRow {
        ...
        redis_used_memory_bytes: redis.used_memory_bytes,
        redis_used_memory_rss_bytes: redis.used_memory_rss_bytes,
        redis_used_memory_dataset_bytes: redis.used_memory_dataset_bytes,
        aof_current_size_bytes: redis.aof_current_size_bytes,
        namespace_keys,
...
pub async fn run_cell(spec: &CellSpec, layout: &FixtureLayout) -> Result<RawRow, String> {
    let prefix = layout.cell_prefix(spec);
    let store = open_store(layout.redis_url(), prefix.clone()).await?;
    ...
    if matches!(
        ...
        let reopened_store = open_store(layout.redis_url(), prefix.clone()).await?;
    ...
    let redis = redis_stats(layout.redis_url()).await?;
    let namespace_keys = namespace_key_count(layout.redis_url(), &prefix).await?;
    Ok(make_row(
        spec,
        &outcome,
        &stats,
...
async fn run_put(store: RedisStore, base: &Tree, spec: &CellSpec) -> Result<Outcome, String> {
...
async fn run_batch(store: RedisStore, base: &Tree, spec: &CellSpec) -> Result<Outcome, String> {
...
async fn run_query(store: RedisStore, base: &Tree, spec: &CellSpec) -> Result<Outcome, String> {
    let ids = read_ids(spec.pattern, spec.records, spec.read_samples);
...
async fn run_scan(store: RedisStore, base: &Tree, spec: &CellSpec) -> Result<Outcome, String> {
...
async fn run_diff(store: RedisStore, base: &Tree, spec: &CellSpec) -> Result<Outcome, String> {
...
async fn run_merge(store: RedisStore, base: &Tree, spec: &CellSpec) -> Result<Outcome, String> {
