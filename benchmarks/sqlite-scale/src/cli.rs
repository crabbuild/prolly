...
pub const USAGE: &str = "usage: prolly-sqlite-scale-bench [--profile smoke|full] [--output PATH] [--sizes LIST] [--runs N] [--operations LIST] [--patterns LIST] [--changes N|auto] [--read-samples N] [--min-free-gb N] [--keep-fixtures] [--revision REV] [--dirty|--clean]";
    ...
    S: AsRef<str>,
{
    ...
    let mut config = match profile.as_str() {
        "smoke" => RunConfig::smoke(PathBuf::from("performance-results/sqlite/baseline/smoke")),
        "full" => RunConfig::full(
            PathBuf::from("performance-results/sqlite/baseline"),
            "unknown".to_string(),
            true,
        ),
