...
impl RunConfig {
    ...
    pub fn validate(&self) -> Result<(), String> {
        if self.output.as_os_str().is_empty() || self.revision.is_empty() {
            return Err("output and revision must be non-empty".to_string());
        }
        ...
        for records in &self.sizes {
            ...
            if self.operations.contains(&Operation::Merge) && changes % 2 != 0 {
